...
void ASAM::Init(){
	/******** SAM parameters ********/
	volumes.assign(NUMPAT, 0.5);      /* volume (area) of then-part set in SAM */
	centroids.assign(NUMPAT, 200);    /* then-part set centroid in SAM */
	...
	F.assign(NUMDES, 200);   /* SAM approximator values */
	Var.assign(NUMDES, 1);   /* SAM approximator values variance */
